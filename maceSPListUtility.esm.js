/**
 * ESM variant of maceSPListUtility.js
 * - Uses `import.meta.url` (when available) to determine SITE_BASE
 * - Exports named functions for direct ESM imports
 */
/* eslint-disable no-console */
'use strict'
console.log('Loading ESM Module maceSPListUtility.esm.js')
const SemVer = '2.1.1'

// Module-scoped site base
let SITE_BASE

// Attempt to detect SITE_BASE from script URL for classic script or from module URL for ESM
try {
  if (typeof document !== 'undefined' && document?.currentScript) {
    const src = document.currentScript.src
    const reFindSite = /(.+)\/siteAssets\/(.*)/i
    if (reFindSite.test(src)) {
      const match = reFindSite.exec(src)
    ...
    if (reFindSite.test(src)) {
      const match = reFindSite.exec(src)
      console.log(`maceSPListUtility (ESM) dynamically determined its site base to be ${match[1]}`)
      SITE_BASE = match[1]
    }
  }
} catch (e) {
  // Best-effort detection only
  console.warn('Site base detection failed:', e?.message || e)
}
...
// Utility helpers expecting Ramda `R` available globally
...
const convertIsoDateTextToDate = function (dateText) {
...
function popLastFailedRequest() { return cacheFailedRequests.pop() }
function getCountFailedResponse() { return cacheFailedRequests.length }
